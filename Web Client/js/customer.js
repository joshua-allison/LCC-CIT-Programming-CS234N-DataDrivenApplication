class CustomerPage {
  ...
  constructor() {
    ...
    this.server = "https://localhost:44391/api"
  ...
  fetchStates() {
    //take the originaly server address, append '/states', get the response, and parse it as JSON text
    fetch(`${this.server}/states`)
    .then(response => response.json())
    // then, with the data returned...  
    .then(data => { 
      // ... if the data comes back, and it is not an array with a length unequal to 0
      if (data.length == 0) {
        // then the request did not load the states properly.
        alert("Can't load states.  Can not add or edit customers without state inforamtion.");
      }
      // If it did loads the states properly,
      else {
        // assign the data to the states object,
        this.state.states = data;
        // and call the loadStates method.
        this.loadStates();
      }
    })
    .catch(error => {
      // if you see this, the fetch didnt work.
  ...
  onFindCustomer(event) {
    //clears any default behavior that is triggered from the 'event'
    event.preventDefault();
    //if the id of the customer is not blank, then ...
    if (this.$customerId.value != "") {
      // make the customer object's id equal to the customer's id set by the user in the ui element,
      this.state.customerId = this.$customerId.value;
      // and use that customer object's id to make an api call for info on that customer,
      fetch(`${this.url}/${this.state.customerId}`)
      //the resonse should ber formatted into JSON text
      .then(response => response.json())
      .then(data => { 
        // once it's formatted, if the result is not found,
        if (data.status == 404) {
          // then tell the user that a customer with that ID does not exist.
          alert('That customer does not exist in our database'); 
        }
        //otherwise (if the result is found),
        else {
          // set the customer object equal to the returned result customer
          this.state.customer = data;
          // and fill the ui with the data from the retrieved customer.
          this.fillCustomerFields();
          this.enableButtons("found");
        }
      })
      .catch(error => {
        // if you're reading this, the fetch didn't work.
        alert('There was a problem getting customer info!'); 
      });
    }
    else {
      // if the customer search did not yield a customer, after alerting the user, clear all the fields associated with customer data.
  ...
  onDeleteCustomer(event) {
    ...
    if (this.state.customerId != "") {
      // get the customer from the DB who's id matches the id stored in the app's customer object, and delete that customer
  ...
  onSaveCustomer(event) {
    ...
    if (this.state.customerId == "") {
      fetch(`${this.url}`, {
        method: 'POST', 
        //this looks like the text version of making a postman request.
        body: JSON.stringify({
          customerId: 0, 
